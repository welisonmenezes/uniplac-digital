...
def index():
    ...
    news = Post.query.filter(and_(Post.entry_date <= current_date, Post.departure_date >= current_date, Post.genre=='news', Post.status=='approved')).order_by(desc(Post.id)).limit(10)
    ads = Post.query.filter(and_(Post.entry_date <= current_date, Post.departure_date >= current_date, Post.genre=='ad', Post.status=='approved')).order_by(desc(Post.id)).limit(6)
    notices = Post.query.filter(and_(Post.entry_date <= current_date, Post.departure_date >= current_date, Post.genre=='notice', Post.status=='approved')).order_by(desc(Post.id)).limit(6)
